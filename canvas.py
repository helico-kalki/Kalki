...
class Canvas(QFrame):
    ...
    def set_zoom_level(self, zoom: float):
        self.zoom_level = max(0.01, min(zoom, 50.0))
        self.update()

    def zoom_in_at_point(self, point=None):
        if point is None:
        ...
        # Adjust pan offset to keep the point under the cursor
        zoom_factor = self.zoom_level / old_zoom
        self.pan_offset = QPoint(
            int(point.x() - (point.x() - self.pan_offset.x()) * zoom_factor),
            int(point.y() - (point.y() - self.pan_offset.y()) * zoom_factor)
        )
        self.update()

    def zoom_out_at_point(self, point=None):
        if point is None:
            # Default to center if no point provided
            point = QPoint(self.width() // 2, self.height() // 2)
    ...
    def get_scaled_mouse_pos_from_point(self, point):
        adjusted_pos = point - self.pan_offset
        return QPoint(int(adjusted_pos.x() / self.zoom_level), int(adjusted_pos.y() / self.zoom_level))
    ...
    def mousePressEvent(self, event: QMouseEvent):
        ...
        if self.selection_mode and (not self.selection_rect and not self.selection_path):
            self.selection_active = True
            self.selection_start = self.get_scaled_mouse_pos(event)
            if self.selection_mode == "lasso":
                self.selection_path = QPainterPath()
                # Convert QPoint to QPointF for QPainterPath
                self.selection_path.moveTo(QPointF(self.selection_start.x(), self.selection_start.y()))
            return
        ...
        if self.eyedropper_mode:
            ...
            if 0 <= pos.x() < self.image.width() and 0 <= pos.y() < self.image.height():
                color = QColor(self.image.toImage().pixel(pos))
    ...
    def mouseMoveEvent(self, event: QMouseEvent):
        ...
        if self.selection_active and self.selection_mode:
            ...
            if self.selection_mode == "rect" or self.selection_mode == "ellipse":
                ...
                # Convert QPoint to QPointF for QPainterPath
                self.selection_path.lineTo(QPointF(current.x(), current.y()))
            self.update()
            return
    ...
    def mouseReleaseEvent(self, event: QMouseEvent):
        if self.move_mode:
            ...
            return
        if self.selection_active and self.selection_mode:
            if self.selection_mode == "lasso" and self.selection_start:
                # Close the lasso path by connecting back to start
                self.selection_path.lineTo(QPointF(self.selection_start.x(), self.selection_start.y()))
    ...
    # Get mouse position for zoom centering
        ...
        if event.angleDelta().y() > 0:
            self.zoom_in_at_point(mouse_pos)
        else:
            self.zoom_out_at_point(mouse_pos)
    ...
    def apply_gradient(self, color1=QColor("white"), color2=QColor("blue")):
        ...
        # Determine the gradient area and create appropriate gradient
        if self.selection_mode == "rect" and self.selection_rect:
            gradient = QLinearGradient(
                QPointF(self.selection_rect.topLeft()), 
                QPointF(self.selection_rect.topRight())
            )
            ...
            # Get bounding rect of lasso selection for gradient direction
            bounding_rect = self.selection_path.boundingRect()
            gradient = QLinearGradient(
            ...
            gradient.setColorAt(0, color1)
            gradient.setColorAt(1, color2)
            painter.fillPath(self.selection_path, gradient)
        else:
            # Apply to entire canvas if no selection
            gradient = QLinearGradient(0.0, 0.0, float(self.image.width()), 0.0)
            gradient.setColorAt(0, color1)
            gradient.setColorAt(1, color2)
    ...
    def fill_selection(self, color: QColor):
        ...
        if self.selection_mode == "rect" and self.selection_rect:
            ...
            painter.fillPath(self.selection_path, color)
        else:
            # Fill entire canvas if no selection
    ...
    # Save state for undo
        ...
        if self.selection_mode == "rect" and self.selection_rect:
            ...
            selected_image = self.image.toImage().copy(self.selection_rect)
            pil_img = self._qimage_to_pil(selected_image)
            processed_pil_img = filter_func(pil_img)
            processed_q_img = processed_pil_img.toqimage()
            
            # Paint the processed image back to the original position
            painter = QPainter(self.image)
            ...
            mask_size = self.selection_rect.size()
            mask = QPixmap(mask_size)
            mask.fill(Qt.GlobalColor.transparent)
            mask_painter = QPainter(mask)
            mask_painter.setBrush(Qt.GlobalColor.white)
            mask_painter.setPen(Qt.PenStyle.NoPen)
            mask_painter.drawEllipse(QRect(QPoint(0, 0), mask_size))
            mask_painter.end()
            
            # Extract selected area and apply mask
            selected_pixmap = self.image.copy(self.selection_rect)
            selected_pixmap.setMask(mask.createMaskFromColor(Qt.GlobalColor.transparent))
            
            # Convert to PIL, apply filter, convert back
            selected_image = selected_pixmap.toImage()
            pil_img = self._qimage_to_pil(selected_image)
            processed_pil_img = filter_func(pil_img)
            processed_pixmap = self._pil_to_qpixmap(processed_pil_img)
            
            # Apply the mask to the processed image
            processed_pixmap.setMask(mask.createMaskFromColor(Qt.GlobalColor.transparent))
            
            # Paint back to canvas
            painter = QPainter(self.image)
            painter.setCompositionMode(QPainter.CompositionMode.CompositionMode_SourceOver)
            painter.drawPixmap(self.selection_rect.topLeft(), processed_pixmap)
            painter.end()
            
        elif self.selection_mode == "lasso" and self.selection_path:
            # Get bounding rect of lasso selection
            bounding_rect = self.selection_path.boundingRect().toRect()
            ...
            mask_painter.setBrush(Qt.GlobalColor.white)
            mask_painter.setPen(Qt.PenStyle.NoPen)
            # Translate path to mask coordinates - fix the type conversion
            translated_path = QPainterPath(self.selection_path)
            translated_path.translate(QPointF(-bounding_rect.topLeft().x(), -bounding_rect.topLeft().y()))
            mask_painter.fillPath(translated_path, Qt.GlobalColor.white)
            mask_painter.end()
            
            # Extract selected area and apply mask
            selected_pixmap = self.image.copy(bounding_rect)
            selected_pixmap.setMask(mask.createMaskFromColor(Qt.GlobalColor.transparent))
            ...
            painter = QPainter(self.image)
            painter.setCompositionMode(QPainter.CompositionMode.CompositionMode_SourceOver)
            painter.drawPixmap(bounding_rect.topLeft(), processed_pixmap)
            painter.end()
            
        else:
            # Apply to entire image if no selection
            pil_img = self._qimage_to_pil(self.image.toImage())
            processed_img = filter_func(pil_img)
            self.image = self._pil_to_qpixmap(processed_img)
    ...
    def apply_blur(self, radius):
        ...
        self._apply_filter_to_selection(blur_filter)

    def apply_emboss(self):
        self._apply_filter_to_selection(lambda img: img.filter(ImageFilter.EMBOSS))
    ...
    def apply_sharpen(self):
        self._apply_filter_to_selection(lambda img: img.filter(ImageFilter.SHARPEN))
